...
class GradientFilter:
    def __init__(self, abs_ksize = 3, mag_ksize = 9, dir_ksize = 15, abs_thresh=(20,100),mag_thresh=(30,100),dir_thresh=(0.8,1.3), color_thresh=(90,255)):
    ...
    def process(self, img):
        ...
        l_gradx = self.abs_sobel_threshold(img,1, orient='x', sobel_kernel=self.abs_ksize, thresh=(8,255))
        l_grady = self.abs_sobel_threshold(img,1, orient='y', sobel_kernel=self.abs_ksize, thresh=(8,255))
        l_dir_binary = self.dir_threshold(img,1, sobel_kernel=self.dir_ksize, thresh=self.dir_thresh)
        l_color_binary = self.color_threshold(img,1, (140,255))
        ...
        combined[ ((l_grady==1)&(l_gradx==1)&(l_dir_binary==1)&(l_color_binary==1))|((s_gradx==1)&(s_dir_binary==1)&(s_color_binary==1)) ] = 1#| (color_binary==1)
#&
# 
        return combined